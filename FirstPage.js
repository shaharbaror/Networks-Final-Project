...
async function FetchFirstData(){
    ...
    console.log(data);
    document.getElementById("players_done").innerHTML = data.done;
    ...
    setInterval(async ()=> {
        const fetched = await fetch("firstpage?s=f&a=gettime");
        if (fetched.ok) {
...
function SubmitMeme() {
    ...
    try{
        ...
        //     }
        //     else {
        //         //window.location.href = "/" +`${lobby}/waitingpage.html`;
        //     }
        // }});
        PostMeme(post);
...
async function PostMeme(post) {
    const response = await fetch("firstpage?s=t",{
        method:"POST",
        headers:{"Content-Type":"application/json;charset=utf-8"},
        body:JSON.stringify(post)
    });
